...
type Props = {
  configId: string
  imgUrl: string
  imgDimensions: Pick<Dimension, 'width' | 'height'> | Dimension
}
...
const DesignConfigurator = ({ configId, imgUrl, imgDimensions }: Props) => {
  ...
  async function saveConfig() {
    ...
    try {
      const { top: caseTop, left: caseLeft, width, height }: DOMRect = phoneCaseRef.current!.getBoundingClientRect(); // ? getBoundingClientRect() returns the size of the element and its position relative to the viewport
      const { top: divTop, left: divLeft }: Omit<DOMRect, 'width' | 'height'> = containerRef.current!.getBoundingClientRect(); // ? getBoundingClientRect() returns the size of the element and its position relative to the viewport
    ...
    return (
      <div className="relative mt-20 grid grid-cols-1 lg:grid-cols-3 mb-20 pb-20">
        <div className="relative h-[37.5rem] mb-4 overflow-hidden col-span-2 w-full max-w-4xl flex items-center justify-center rounded-lg border-2 border-dashed border-gray-300 p-12 text-center focus:outline-none focus:ring-2 focus:ring-primary focus:ring-offset-2">
          <div
            ref={containerRef}
          ...
          <Rnd
            ...
            onDragStop={(_, data: Coord) => {
              setRenderedPosition(data) // ? {x,y} is the data that we get from the onDragStop event
            }}
          >
